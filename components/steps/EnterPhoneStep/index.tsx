...
export const EnterPhoneStep = () => {
  ...
  const onSubmit = async () => {
    try {
      ...
      const smsCode = await Axios.get(`/auth/sms?phone=${values.value}`)
      if(smsCode){
  ...
  return (
    <div className={styles.block}>
      ...
      <WhiteBlock className={clsx('m-auto mt-30', styles.whiteBlock)}>
        <div className={clsx('mb-3onNextStep()0', styles.input)}>
        ...
        <Button className='mt-10' onClick={onSubmit} disabled={isLoading || nextDisabled} >
          {isLoading ? (
            'Sending...'
          ) : (
            <>
